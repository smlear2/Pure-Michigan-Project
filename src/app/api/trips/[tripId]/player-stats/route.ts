...
import { computeSkinsForRound, HandicapConfig } from '@/lib/golf'
...
export async function GET(
  ...
  try {
    ...
    // Skins aggregation — compute per round using WHS skins handicaps
    ...
    for (const round of skinsRounds) {
      ...
      const { playerPayouts } = computeSkinsForRound(
        round.format,
        round.tee,
        roundScores.map(s => ({
          holeId: s.holeId,
